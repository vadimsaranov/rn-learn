...
class NativeDatabase {
  ...
  constructor(_databaseName: string, _options?: SQLiteOpenOptions, serializedData?: Uint8Array) {
    if (serializedData !== null && serializedData !== undefined) {
      const serializedDataString = Array.from(serializedData)
        .map((b) => String.fromCharCode(b))
        .join('');
      this.sqlite3Db = new sqlite3(serializedDataString);
    } else {
...
class NativeStatement {
  ...
  public stepAsync = (_database: NativeDatabase): Promise<any> => {
    if (this.iterator === null) {
      if (this.sqlite3Stmt === null) {
        throw new Error('sqlite3Stmt is null');
      }
  ...
  public getAllAsync = (_database: NativeDatabase) => Promise.resolve(this._allValues());
  ...
  public stepSync = (_database: NativeDatabase): any => {
    if (this.iterator === null) {
      this.iterator = this.sqlite3Stmt?.iterate() || null;
